...
from llm.memory import chat_memory
...
def ask_question(body: AskRequest, user_id: str = Depends(get_user_id)):
    ...
    try:
        # 0. Check Cache
        cached_res = llm_cache.get(query)
        if cached_res:
            return cached_res

        # 1. Inject Conversation History (Exceptional: Memory Support)
        history_context = chat_memory.get_context(user_id)
        contextual_query = f"{history_context}\nCurrent User Question: {query}" if history_context else query

        # 2. Embed the query
        query_embedding = get_single_embedding(query)

        # 3. Retrieve top relevant chunks from FAISS (Exceptional: Hybrid Search)
        top_chunks = search(query_embedding, top_k=5, query_text=query)

        # 4. Generate grounded answer via LLM
        result = generate_answer(contextual_query, top_chunks)

        # 5. Save to Cache and Memory
        llm_cache.set(query, result)
        chat_memory.add_message(user_id, "user", query)
        chat_memory.add_message(user_id, "assistant", result["answer"])
...
@router.post("/delete-file", tags=["System"])
def delete_file(doc_id: str, user_id: str = Depends(get_user_id)):
    """Remove a specific file from the index."""
    delete_file_from_index(doc_id)
    return {"status": "success", "message": f"File {doc_id} removed from index."}
